...
export class TileWindowManagerService implements TileWindowManager {
  ...
  constructor() {
    const handleSubscription = async (data: TileManagerProxyMessage) => {
      switch (data.type) {
        case 'openWindow':
          await this.openWindow(data.options);
          break;
  ...
  openWindow = async (options: WindowOption) => {
    ...
    const lastWindowMonitor = await this.getWindowMonitor(
      this.lastWindow || this.mainWindow || fin.Window.getCurrentSync(),
    );
    ...
    if (this.lastWindowBounds) {
      ...
      windowBounds = {
        top,
        left,
        bottom: top + options.defaultHeight,
        right: left + options.defaultWidth,
      ...
      windowBounds = await this.getCenteredWindowBounds(
        window,
        options.defaultWidth,
        options.defaultHeight,
        lastWindowMonitor.bounds,
      );
    }
  ...
  private getCenteredWindowBounds = async (
    window: _Window,
    width: number,
    height: number,
    monitorBounds: Rect,
  ...
  private getWindowMonitor = async (window: _Window): Promise<WindowMonitor> => {
    ...
    const getMonitor = (anchor: AnchorType) => {
      ...
      if (this.isInMonitor(windowBounds, monitorInfo.primaryMonitor, anchor)) {
        result = {
          primary: true,
          name: monitorInfo.primaryMonitor.name,
          bounds: monitorInfo.primaryMonitor.monitor.dipRect,
        };
      } else {
  ...
  private isInMonitor = (winBounds: Bounds, monitorData: MonitorDetails, anchor: AnchorType = 'top-left') => {
    if (anchor === 'top-left') {
      return (
        winBounds.left >= monitorData.monitorRect.left &&
    ...
    if (anchor === 'top-right') {
      return (
        winBounds.right <= monitorData.monitorRect.right &&
